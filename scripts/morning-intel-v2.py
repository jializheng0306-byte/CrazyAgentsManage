...
晨间情报采集脚本 v2
每日08:30执行
信息源: arxiv + Hacker News + GitHub Blog + TechCrunch
特点: 标题和简介翻译成中文
"""
...
def translate_to_chinese(text):
    """简单翻译（实际应用中可使用翻译API）"""
    # 这里使用简单的关键词替换作为示例
    # 实际应用中应该使用翻译API如Google Translate或DeepL
...
def generate_summary(papers, rss_items):
    """生成情报摘要（中文版）"""
...
def send_to_feishu(papers, rss_items):
    """发送到飞书群"""
...
def main():
    ...
    print("\n📤 发送到飞书群...")
    try:
        send_ok = send_to_feishu(papers, rss_items)
        if send_ok:
            print("  ✅ 飞书群推送成功")
        else:
            print("  ❌ 飞书群推送失败")
