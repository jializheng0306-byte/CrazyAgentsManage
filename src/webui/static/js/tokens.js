...
async function loadAllTokenData() {
  try {
    ...
    const resp = await fetch('./api/tokens/stats', { signal: controller.signal });
...
function renderTimeRangeSelector() {
  const container = document.querySelector('.time-range-container');
  if (!container) return;
  ...
  container.innerHTML = '<div class="time-range-selector">' +
    ranges.map(r => `<button class="time-range-btn ${currentTimeRange === r.key ? 'active' : ''}" onclick="setTimeRange('${r.key}')">${r.label}</button>`).join('') +
    '</div>';
}
...
function renderProviderBreakdown(data) {
  ...
  container.innerHTML = entries.map(([name, v]) => {
    ...
    return `<div style="margin-bottom:12px;">
      ...
      <div class="token-bar-track"><div class="token-bar-fill" style="width:${pct}%;background:#10a37f;"></div></div>
    </div>`;
  }).join('');
}
...
function renderAgentBreakdown(data) {
  ...
  container.innerHTML = entries.map(([name, v], i) => {
    ...
    return `<div style="margin-bottom:12px;">
      ...
      <div class="token-bar-track"><div class="token-bar-fill" style="width:${pct}%;background:${colors[i % colors.length]};"></div></div>
    </div>`;
  }).join('');
}
...
async function loadRecentUsage() {
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 60000);
    const resp = await fetch('./api/tokens/recent?limit=10', { signal: controller.signal });
    clearTimeout(timeoutId);
    const data = await resp.json();
    ...
    if (data.length === 0) {
      container.innerHTML = '<div class="empty-state">暂无消耗记录</div>';
      return;
    }
