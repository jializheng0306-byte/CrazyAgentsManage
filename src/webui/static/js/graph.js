...
function renderGraph(data) {
  ...
  if (centerNode) {
    html += `
      <div style="position: absolute; left: ${centerX}%; top: ${centerY}%; transform: translate(-50%, -50%); z-index: 10;">
        <div style="width: 80px; height: 80px; border-radius: 50%; background: linear-gradient(135deg, ${centerNode.gradient.split(',')[0]}, ${centerNode.gradient.split(',')[1] || centerNode.gradient.split(',')[0]}); display: flex; align-items: center; justify-content: center; font-size: 28px; box-shadow: 0 0 20px rgba(102,126,234,0.4); cursor: pointer;" title="${escapeHtml(centerNode.name)}">
          ${centerNode.icon}
        </div>
        <div style="text-align: center; margin-top: 6px; font-size: 11px; color: white; font-weight: 600;">${escapeHtml(centerNode.name)}</div>
        <div style="text-align: center; font-size: 10px; color: #94a3b8;">${centerNode.session_count} 会话</div>
      </div>
    `;
  }
  ...
  agentNodes.forEach((node, idx) => {
    ...
    html += `
      <div style="position: absolute; left: ${x}%; top: ${y}%; transform: translate(-50%, -50%); z-index: 5;">
        <div style="width: 56px; height: 56px; border-radius: 50%; background: linear-gradient(135deg, ${node.gradient.split(',')[0]}, ${node.gradient.split(',')[1] || node.gradient.split(',')[0]}); display: flex; align-items: center; justify-content: center; font-size: 22px; cursor: pointer; position: relative;" title="${escapeHtml(node.name)}">
          ${node.icon}
          <div style="position: absolute; bottom: -2px; right: -2px; width: 12px; height: 12px; border-radius: 50%; background: ${stateColor}; border: 2px solid #0f172a;"></div>
        </div>
        ...
        <div style="text-align: center; font-size: 9px; color: #94a3b8;">${node.session_count} 会话</div>
      </div>
    `;
  });
...
function renderEdgeTypes(data) {
  ...
  container.innerHTML = edgeTypes.map(et => `
    <div style="display: flex; align-items: center; gap: 8px; padding: 6px 0;">
      ...
      <span style="font-size: 13px; color: #cbd5e1;">${et.label}</span>
      <span style="font-size: 11px; color: #64748b; margin-left: auto;">${et.desc} (${et.count})</span>
    </div>
  `).join('');
}
