...
3. `Loop Surface` 已接入真实路由/模板实现，并已承接 `promise-review-cycle` + `morning-intel-cycle` 两条 cycle 对象，以及本地 `feedback input / memory candidate` 最小写面；剩余缺口转为 promotion gate、role isolation 与 executor capability plane integration
